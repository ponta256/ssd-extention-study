...
class FSSD(nn.Module):
    ...
    def __init__(self, phase, cfg, use_pred_module=False):
        ...
        # 1280x768->640x384->320x192->160x96->80x48->40x24->20x12
        ...
        # Fusion Module
        size = 64
        self.fm0 = nn.ModuleList(self.fuse_module(1024, size))          # 80x48 ,1024
        self.fm1 = nn.ModuleList(self.fuse_module(2048, size))          # 40x24 ,2048
        self.fm2 = nn.ModuleList(self.fuse_module(2048, size))          # 20x12 ,2048
        self.fm = [self.fm0, self.fm1, self.fm2]
        self.fm_bn = nn.BatchNorm2d(256*3, affine=True)

        # number of channels in feature extractor
        source = [512, 512, 512, 256, 256]
        box = cfg['box']
        ...
        self.fe = [self.fe0, self.fe1, self.fe2, self.fe3, self.fe4]
        ...
        if self.use_pred_module:
            ...
            self.pm = [self.pm0, self.pm1, self.pm2, self.pm3, self.pm4]
        ...
        source = [1024, 1024, 1024, 1024, 1024]        
        loc_layers = [
    ...
    def forward(self, x):
        ...
        x = self.resnext.conv1(x)       # 640x384
        x = self.resnext.bn1(x)
        x = self.resnext.relu(x)
        x = self.resnext.maxpool(x)     # 320x192
        x = self.resnext.layer1(x)
        x = self.resnext.layer2(x)      # 160x96
        x = self.resnext.layer3(x)      # 80x48
        sources.append(x)
        x = self.resnext.layer4(x)      # 40x24
        sources.append(x)        
        for l in self.extra:
            x = l(x)
        sources.append(x)
        ...
        for k, v in enumerate(self.fm):
            ...
            for l in v:
                x = l(x)
            x = F.interpolate(x, (48,80), mode='bilinear', align_corners=False)
            upsampled.append(x)
