...
class SSD(nn.Module):
    ...
    def __init__(self, phase, cfg, use_pred_module=False):
        ...
        # deconvolution
        self.deconv0 = nn.ConvTranspose2d(source[1], source[0], kernel_size=2, stride=2)
        self.deconv1 = nn.ConvTranspose2d(source[2], source[1], kernel_size=2, stride=2)
        self.deconv2 = nn.ConvTranspose2d(source[3], source[2], kernel_size=2, stride=2)
        self.deconv3 = nn.ConvTranspose2d(source[4], source[3], kernel_size=2, stride=2)
        self.deconv4 = nn.ConvTranspose2d(source[5], source[4], kernel_size=2, stride=2)
        self.deconv5 = nn.ConvTranspose2d(source[6], source[5], kernel_size=2, stride=2)
        self.deconv6 = nn.Conv2d(source[6], source[6], kernel_size=1)
    ...
    def forward(self, x):
        ...
        if self.use_pred_module:
            for i, (x, p, l, c) in enumerate(zip(sources, self.pm, self.loc, self.conf)):
                # deconvolution
                if i == 0:
                    x = x+self.deconv[0](sources[1]+self.deconv[1](sources[2]+self.deconv[2](sources[3])))
                elif i == 1:
                    x = x+self.deconv[1](sources[2]+self.deconv[2](sources[3]))
                elif i == 2:
                    x = x+self.deconv[2](sources[3])
...
def build_ssd(phase, cfg, use_pred_module=False):
    if phase != "test" and phase != "train" and phase != 'stat':
        ...
        return
    return SSD(phase, cfg, use_pred_module)
